...
// User code makes a system call with INT T_SYSCALL. System call 
// number in %eax. Arguments on the stack, from the user call to 
// the C library system call function. The saved user %esp points
// to a saved program counter, and then the first argument.

int badaddr(uint addr) 
{
  const pte_t flags = PTE_P|PTE_U;
  struct proc *curproc = myproc();
  pte_t *pte = walkpgdir(curproc->pgdir,(void*)addr,0);
  return !pte || (*pte & flags) != flags;
}
