...
public:
    ...
    void add(const FP &other, RoundMode mode)
    {
        ...
        int shiftAmount = 60 - significand_size; 
        sigY <<= shiftAmount; 
        sigX <<= shiftAmount; 
        //shift Y's significand to right, based off diff in exponents
        sigY += (1ULL << (significand_size+shiftAmount)); //adding hidden 1 to our sig
        sigY >>= expDiff;

        //set E to X's exponent 
        int32_t e = expX; 
        sigX += (1ULL << (significand_size+shiftAmount)); //adding hidden 1 to our sig
        //set S to X + Y if signs are equal, or X - Y if diff signs
        if (signX == signY) {
            significand = sigX + sigY; 
        }
